 ...
 *   services/<id>.html   one page per service
...
const businessLd = {
  ...
  hasOfferCatalog: {
    ...
    itemListElement: S.services.map((s) => ({
      '@type': 'Offer',
      itemOffered: { '@type': 'Service', name: s.name, description: s.blurb, ...(siteUrl && { url: `${siteUrl}services/${s.id}.html` }) }
    }))
  }
};
...
S.services.forEach((s) => write(`services/${s.id}.html`, servicePage(s)));
...
<body>
  <main class="not-found">
    ...
    <div class="container">
      ...
      <ul class="chips">${S.services.map((s) => `<li><a href="services/${s.id}.html">${esc(s.name)}</a></li>`).join('')}</ul>
    </div>
  </main>
</body>
</html>
...
/* ---------- Sitemap & robots ---------- */
if (siteUrl) {
  const urls = ['', ...S.services.map((s) => `services/${s.id}.html`)];
...
console.log(`Built ${readdirSync(join(root, 'services')).length} service pages, 404.html, ${siteUrl ? 'sitemap.xml, ' : ''}robots.txt`);
